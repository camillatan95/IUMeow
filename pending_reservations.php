...
<br><br><br><br><br><br><br>
    <div class='container'>
      <h2 style="color: #ea7777"> Your Pending Reservations List</h2><br>
...
$sql = "SELECT *
        FROM pending_agreements AS pg, reservations AS r
        WHERE pg.cs_id = r.cat_sitter_id AND pg.rev_id = r.reservation_id AND pg.cs_id = '$cs_user_id' AND pg.co_owner_invoice_status IS NULL ORDER BY pg.rev_id DESC;";
$result = pg_query($sql);
...
End Date <input type='date' class='form-control' name='co_end_date' value='$co_end_date' readonly>
  ...
  <span class='btn btn-warning text-white font-weight-bold'>Pending</span>
  <button type='submit' name='submit_decline' class='btn btn-primary text-white font-weight-bold'>Cancel Pending Reservation</button>
</form>
</div>
</div>
</div>
